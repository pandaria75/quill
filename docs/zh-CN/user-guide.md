...
本文面向**当前 CLI MVP / 参考 Harness**。它说明你今天能运行什么，同时明确 Quill 的目标方向：长期重心是 Quill Core + adapters，先 Hermes，后 OpenCode / Pi。

## 1. Quill 现在能做什么

Quill 当前 MVP 适合这样的使用方式：你在本地创建一个写作工作区，围绕一篇技术博客逐步生成和维护多个 Markdown 文件，并在过程中保留人工修改空间。

因此，本文描述的是**当前参考 Harness**，不是在宣称 CLI 就是 Quill 的最终产品重心。
...
如果你想了解未来架构方向，请结合 `README.md` 与 `docs/target/` 下的目标文档一起阅读。但这些目标文档描述的是产品方向，不代表当前仓库里的 Hermes / OpenCode / Pi 适配器行为已经完整实现。
...
这是**当前 CLI / 参考 Harness** 的事实，不应被理解为“未来的 Quill Core 会拥有提供方凭据或模型账户配置”。
...
在目标的 Core + adapters 架构里，这些环境与运行时问题应由宿主或适配器层负责，而不是由 Quill Core 自己承担。
...
- 当前 CLI 是参考 Harness / 原型，不是长期产品重心
- 当前可用性重点在单篇文章纵向打通，不在平台化
- 未来产品方向是 Quill Core + adapters，先 Hermes，后 OpenCode / Pi
- 目标适配器 / 宿主才应承载运行时、memory、工具、模型选择与凭据处理
